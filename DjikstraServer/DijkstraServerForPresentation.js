...
var graph = require('./DijkstraCal'); // 다익스트라 모듈 로드
        ...
        if (socket.clientState.type === 'APP') {
            ...
            if (socket.clientState.type === 'WheelChair') {
                ...
                while (socket.clientState.buffer.startsWith('1') && socket.clientState.buffer.length >= 6) {
                    ...
                    origin = originLocations.get(originCode) || 'Unknown'; // 맵에서 출발지 찾기
                    console.log('origin: ' + origin + ', destination: ' + destination);

                    // Dijkstra 알고리즘즘
                    //if(!origin && !destination) {
                        if (!graph.nodes.has(origin) || !graph.nodes.has(destination)) {
                            socket.write(`잘못된 노드 입력: ${origin}, ${destination}\r\n`);
                        }
                        else {
                            let result = graph.dijkstra(origin, destination);
                            console.log(`최단 거리: ${result.distance}, 경로: ${result.path.join(' -> ')}, 방향: ${result.directions.join(' -> ')}\r\n`);
                            socket.write(`최단 거리: ${result.distance}, 경로: ${result.path.join(' -> ')}, 방향: ${result.directions.join(' -> ')}\r\n`);
                        }
                    //}
                }
