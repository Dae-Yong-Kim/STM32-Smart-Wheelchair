...
uint32_t max30102_read_heart_rate(max30102_t *sensor)
{
    ...
    // MAX30102 센서에서 심박수 데이터를 읽어오는 로직을 작성해야 합니다.
    // 예시: 데이터 레지스터에서 심박수 정보를 읽는 코드 추가
    // 이 부분은 MAX30102 센서의 데이터를 읽는 방식에 맞춰 작성해야 합니다.

    return heart_rate;
}

uint32_t max30102_get_ir_sample(max30102_t *obj)
{
    // FIFO에서 데이터를 읽어오는 함수 (이미 `max30102_read_fifo`와 비슷한 방식으로 읽을 수 있음)
    uint8_t sample[6];
    max30102_read(obj, MAX30102_FIFO_DATA, sample, 6);
